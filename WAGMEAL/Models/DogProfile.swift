...
extension DogProfile {
    /// このワンちゃんのアレルギーに対応する IngredientFilter セットを返す
    var allergyFilters: Set<IngredientFilter> {
        var filters = Set<IngredientFilter>()
        if allergicChicken ?? false { filters.insert(.chicken) }
        if allergicBeef ?? false { filters.insert(.beef) }
        if allergicPork ?? false { filters.insert(.pork) }
        if allergicLamb ?? false { filters.insert(.lamb) }
        if allergicFish ?? false { filters.insert(.fish) }
        if allergicEgg ?? false { filters.insert(.egg) }
        if allergicDairy ?? false { filters.insert(.dairy) }
        if allergicWheat ?? false { filters.insert(.wheat) }
        if allergicCorn ?? false { filters.insert(.corn) }
        if allergicSoy ?? false { filters.insert(.soy) }
        return filters
    }
}
